...
function Header() {
  ...
  const {update, bedWidth, bedHeight, fileData, currentLayer}  = useContext(GcodeContext);
  const classes = useStyles();

  const processChange = async (data: string) => {
    const {gcode, layers, selectedLayer} = await new GCodeTools(bedWidth, bedHeight, currentLayer).processFile(data);
    update({
      currentGcode: gcode,
      fileData: data,
      layers,
  ...
  const changeLayer = async (layer: string) => {
    const {gcode, selectedLayer} = await new GCodeTools(bedWidth, bedHeight, layer).processFile(fileData);
    update({
      currentGcode: gcode,
      currentLayer: layer,
      filteredFileData: selectedLayer,
